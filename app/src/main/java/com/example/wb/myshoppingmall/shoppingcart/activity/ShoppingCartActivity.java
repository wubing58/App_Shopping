package com.example.wb.myshoppingmall.shoppingcart.activity;
...
public class ShoppingCartActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        if (v == ibShopcartBack) {
            ...
            intent.setClass(this, Shoukuanma.class);
            startActivity(intent);
            Toast.makeText(this, "activity去结算", Toast.LENGTH_SHORT).show();
    ...
    private void showData() {
        CartStorage cartStorage = CartStorage.getInstance(getApplicationContext());

        List<GoodsBean> goodsBeans = cartStorage.getAllData();
        if (goodsBeans != null && goodsBeans.size() > 0) {
            tvShopcartEdit.setVisibility(View.VISIBLE);
            ll_empty_shopcart.setVisibility(View.GONE);
            adapter = new ShoppingCartAdapter(this, goodsBeans, tvShopcartTotal, cartStorage, checkboxAll, cb_all);
